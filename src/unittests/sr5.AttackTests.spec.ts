...
export const shadowrunAttackTesting = (context: QuenchBatchContext) => {
    ...
    describe('Fire Mode Rules', () => {
        it('apply defense modifier per fire mode', () => {
            // Check no modifier
            assert.strictEqual(
                FireModeRules.fireModeDefenseModifier({
        ...
        it('apply attack modifier per fire mode', () => {
            ...
            assert.strictEqual(
                FireModeRules.recoilModifierAfterAttack(
                    {
        ...
        it('reduce the available fire modes', () => {
            assert.lengthOf(
                FireModeRules.availableFireModes({
                    single_shot: true,
                    semi_auto: true,
                    burst_fire: true,
                    full_auto: true,
            ...
            assert.lengthOf(
                ...
                1,
            ); // per default rules only one single shot mode
    ...
    describe('CombatRules', () => {
        ...
        const getCharacterWithArmor = async (
            armorValue: number,
            {
                hardened = false,
            }: {
