...
export class JournalEnrichers {
    ...
    static setEnrichers() {
        ...
        CONFIG.TextEditor.enrichers.push({
            pattern: new RegExp(
                `\\@(${JournalEnrichers.actorKeywords.join('|')})${opening}([a-z]+)${threshold}${closing}`,
                'g',
            ),
            enricher: (match, options) => {
                ...
                return $(
                    `<a class="sr5-roll-request" data-request="${type}" data-skill="${rollEntity}" data-threshold=${threshold}><em class="fas fa-dice"></em>${rollEntityName} ${threshold}</a>`,
                )[0];
            },
        });
    }
