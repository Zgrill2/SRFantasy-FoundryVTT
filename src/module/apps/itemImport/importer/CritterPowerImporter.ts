...
export class CritterPowerImporter extends DataImporter<Shadowrun.CritterPowerItemData, Shadowrun.CritterPowerData> {
    ...
    public override unsupportedCategories = ['Emergent'];
    ...
    async Parse(chummerPowers: object, setIcons: boolean): Promise<Item> {
        ...
        const folder = await ImportHelper.GetFolderAtPath(
            `${Constants.ROOT_IMPORT_FOLDER_NAME}/${game.i18n.localize('TYPES.Item.critter_power')}`,
            true,
        );
        ...
        for (const chummerCritterPower of chummerCritterPowers) {
            ...
            const item = parser.Parse(
                chummerCritterPower,
                this.GetDefaultData({ type: parserType }),
