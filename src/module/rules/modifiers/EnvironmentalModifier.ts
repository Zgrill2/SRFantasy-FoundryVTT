...
export class EnvironmentalModifier extends SituationModifier {
    ...
    activeLevels(values: Number[]): Record<string, number> {
        return {
            light: values.reduce(
                (count: number, value: number) => (value === this.levels.light ? count + 1 : count),
                0,
            ) as number,
            moderate: values.reduce(
                (count: number, value: number) => (value === this.levels.moderate ? count + 1 : count),
                0,
            ) as number,
            heavy: values.reduce(
                (count: number, value: number) => (value === this.levels.heavy ? count + 1 : count),
                0,
            ) as number,
            extreme: values.reduce(
                (count: number, value: number) => (value === this.levels.extreme ? count + 1 : count),
                0,
            ) as number,
        };
    }
    ...
    override _calcActiveTotal(): number {
        ...
        if (this.applied.active.value) return this.applied.active.value;
